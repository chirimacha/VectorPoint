...
Login <- function(username, password) {
  ...
  userQuery <- sprintf("SELECT * FROM %s WHERE UPPER(NAME_USER) COLLATE latin1_general_cs LIKE UPPER('%s')", dbGlobalConfig$userTable, username)
...
read_past_inspections <- function(databaseName=dbGlobalConfig$databaseName, 
  ...
  query <-sprintf("SELECT FECHA, UNI_CODE, MAX(DATETIME), USER_NAME, STATUS_INSPECCION, TOT_INTRA, TOT_PERI, RASTROS, PREDICTED_PROBAB, PREDICTED_PROBAB_MEAN FROM %s WHERE TEST_DATA = '0' GROUP BY UNI_CODE DESC", TABLE_NAME)
